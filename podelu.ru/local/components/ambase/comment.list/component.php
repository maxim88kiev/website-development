...
if($this->startResultCache(false, array($arrFilter, ($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups()))))
{
	...
	while($arSection = $rsSections->GetNext())
	{
        ...
        $commentIdList[] = $arSection['ID'];
	}

	$arCommentRates = [];
	if (!empty($commentIdList)) {
        $commentId = implode(',', $commentIdList);
        $sql = "select * from ambase_podelu_comment_rates where comment_id in ($commentId)";
        $dbCommentRates = $DB->Query($sql);
        while($arCommentRate = $dbCommentRates->Fetch())
        {
            if (empty($arCommentRates[$arCommentRate['comment_id']])) {
                $arCommentRates[$arCommentRate['comment_id']]['LIKES'] = 0;
                $arCommentRates[$arCommentRate['comment_id']]['DISLIKES'] = 0;
                $arCommentRates[$arCommentRate['comment_id']]['IS_LIKED'] = false;
                $arCommentRates[$arCommentRate['comment_id']]['IS_DISLIKED'] = false;
            }

            if ($arCommentRate['value']) {
                $arCommentRates[$arCommentRate['comment_id']]['LIKES'] += 1;
            } else {
                $arCommentRates[$arCommentRate['comment_id']]['DISLIKES'] += 1;
            }


            if (
                $arCommentRate['ip'] === $_SERVER['REMOTE_ADDR'] &&
                $arCommentRate['user_agent'] === $_SERVER['HTTP_USER_AGENT']
            ) {
                if ($arCommentRate['value']) {
                    $arCommentRates[$arCommentRate['comment_id']]['IS_LIKED'] = true;
                } else {
                    $arCommentRates[$arCommentRate['comment_id']]['IS_DISLIKED'] = true;
                }
            }
        }
    }
	...
	foreach ($arResult["SECTIONS"] as &$arSection)
	{
		...
		$rates = $arCommentRates[$arSection['ID']];
		if (!empty($rates)) {
            $arSection['LIKES'] = $rates['LIKES'];
            $arSection['DISLIKES'] = $rates['DISLIKES'];
            $arSection['IS_LIKED'] = $rates['IS_LIKED'];
            $arSection['IS_DISLIKED'] = $rates['IS_DISLIKED'];
        } else {
            $arSection['LIKES'] = 0;
            $arSection['DISLIKES'] = 0;
            $arSection['IS_LIKED'] = false;
            $arSection['IS_DISLIKED'] = false;
        }
