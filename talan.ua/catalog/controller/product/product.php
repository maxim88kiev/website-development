	...
	public function index() {
		...
		if ($product_info) {
			...
			if (!empty($this->session->data['user_id']) && !empty($this->session->data['token'])) {
				$data['admin_link']='/admin/index.php?route=catalog/product/edit&token='.$this->session->data['token'].'&product_id='.$product_id;
			}else{
				$data['admin_link']='';
			}
			if (isset($this->request->get['path'])) {
			...
			$data['text_main_retail'] = $this->language->get('text_main_retail');
			...
			$data['attribute_groups'] = $this->model_catalog_product->getProductAttributes($this->request->get['product_id']);
					...
					// XD Stickers start
						...
						if ($xd_stickers['status']) {
							...
							$custom_xd_stickers_id = $this->model_extension_module_xd_stickers->getCustomXDStickersProduct($this->request->get['product_id']);
