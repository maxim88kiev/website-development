                ...
                <div role="form" id="fastorder">
                    <fieldset>
                        ...
                        <div class="form-group input-group-sm">
                            ...
                            <div class="fo-quantity" onchange="foShowOption(this)">
                                <div>
                                    <input class="form-control" id="count<?php echo $product_id;?>" name="count<?php echo $product_id;?>" placeholder="<?php echo $text_fastorder_input_count_placeholder;?>" type="radio" value='1' checked> <p>&lt; 50</p>
                                </div>
                                <div>
                                    <input class="form-control" id="count<?php echo $product_id;?>" name="count<?php echo $product_id;?>" placeholder="<?php echo $text_fastorder_input_count_placeholder;?>" type="radio" value='2'> <p>&gt; 50</p>
                                </div>
                            </div>
                        </div>
...
<script>
    ...
    var k = 1;
    var firmNameOption = document.createElement("div");
    firmNameOption.id = "fo-firmname";
    firmNameOption.innerHTML = '<label for="txt_phone"><?php echo $text_fastorder_firmname;?></label><input type="text" class="form-control" id="firmname<?php echo $product_id;?>" name="firmanme<?php echo $product_id;?>" placeholder="<?php echo $text_fastorder_firmname_placeholder;?>" required>'

    function foShowOption(obj){
        if(k == 1){ 
            $(obj).after(firmNameOption); 
            k = 2; 
        }
        else{ 
            firmNameOption.remove(); 
            k = 1;
        }
    }
    ...
    $('#btn_submit<?php echo $product_id;?>').on('click', function() {
        ...
        if($('#firmname<?php echo $product_id;?>').val()){
            data['firmname'] = $('#firmname<?php echo $product_id;?>').val();
        }
        else{
            data['firmname'] = "";
        }
        ...
        $.ajax({
            ...
            product_name: data['product_name'], price: data['price'], count: data['count'], total: data['total'], product_id: data['product_id'], product_link: data['product_link'], firmname: data['firmname']},
            dataType: 'json',
            beforeSend: function() {
                // Do form valdation
