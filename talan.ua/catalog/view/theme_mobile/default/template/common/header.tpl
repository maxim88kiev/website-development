...
<?php // Begin Themes Scripts ----
	...
	$this->soconfig->addJs(URL_TEMPLATE.'default/js/so.custom.js');
	$this->soconfig->addJs(URL_TEMPLATE.'default/js/common.js');
...
<?php
	//Render a class Body
	if($store_id){
		$mobile['mobilelayout'] = $store_id ;
		$mobile['mobileheader'] = $store_id ;
	}
	$cls_body  =  $class .' ';
	$cls_body .=  $direction.' ' ;
...
<body class="<?php echo $cls_body;?>">
	...
	<?php 
		//Render Panel Left
		include(DIR_TEMPLATE.'default/template/soconfig/panel_bar.tpl');
	?>
	...
	if($mobile['barnav']){
		if(isset($mobile['mobileheader']) ){
			...
			switch ($mobile['mobileheader']) {
				case "0":
					$header1 = DIR_TEMPLATE.'default/template/header/header1.tpl';
					if (file_exists($header1)) include($header1);
					else echo $header_alert; 
					break;
				case "1":
					$header2 = DIR_TEMPLATE.'default/template/header/header2.tpl';
					if (file_exists($header2)) include($header2);
					else echo $header_alert; 
					break;
				case "2":
					$header3 = DIR_TEMPLATE.'default/template/header/header3.tpl';
					if (file_exists($header3)) include($header3);
					else echo $header_alert; 
					break;
			}
		}else{
			include(DIR_TEMPLATE.'default/template/header/header1.tpl');
		}
	}
	?>
