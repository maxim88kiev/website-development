...
class ModelExtensionModuleSeoTagsGenerator extends Model {
	...
	// Поэтому в админке товара использую $this->extension_module_seo_tags_generator->getProductAttributes($this->request->get['product_id'])
	...
	public function getMinPriceInCat($category_id) {
		...
		$sql = "SELECT"
			...
			. " WHERE p2c.category_id IN (SELECT cp.category_id FROM " . DB_PREFIX . "category_path cp WHERE path_id = '" . (int)$category_id . "')"
			. " AND pd2.price > '0' AND pd2.quantity = '1' AND ((pd2.date_start = '' OR pd2.date_start = '0000-00-00' OR pd2.date_start < NOW()) AND (pd2.date_end = '' OR pd2.date_end = '0000-00-00' OR pd2.date_end > NOW()))"
