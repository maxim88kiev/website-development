...
class ControllerStartupSeoUrl extends Controller {
	public function index() {
      ...
      */
      ...
      if ($this->config->get('mlseo_search') && !empty($parts[0]) && !empty($parts[1]) && $parts[0] === $this->config->get('mlseo_fpp_search_'.$this->config->get('config_language_id'))) {
        $this->request->get['route'] = 'product/search';
        $this->request->get['search'] = str_replace('-', ' ', $parts[1]);
      ...
      foreach ($parts as $part) {
        ...
        if ($this->config->get('mlseo_store_mode') && $this->config->get('mlseo_disable_other_store_links')) {
          ...
          if ($partsIteration == count($parts)) {
            ...
            foreach ($query->rows as $checkStoreLang) {
              if ($checkStoreLang['language_id'] == $this->config->get('config_language_id')) {
            ...
            if ($this->config->get('mlseo_enabled') && $this->config->get('mlseo_absolute') && $query->num_rows > 1) {
              ...
              if ($this->config->get('mlseo_fpp_brand_parent') && isset($this->request->get['route']) && $this->request->get['route'] == 'product/manufacturer') {
                unset($this->request->get['route']);
					...
					if (empty($this->request->get['route']) && $url[0] != 'route' && isset ($query->row['query']) && $query->row['query'] && $url[0] != 'information_id' && $url[0] != 'manufacturer_id' && $url[0] != 'category_id' && $url[0] != 'product_id') {
        ...
        if (isset($this->request->get['route']) && $this->config->get('mlseo_redirect_canonical') && !in_array($this->request->get['route'], array('account/login', 'error/not_found', 'product/search')) && !isset($this->request->get['mfp']) && !isset($this->request->get['sort']) && !isset($this->request->get['limit']) && !empty($_SERVER['HTTP_HOST'])) {
