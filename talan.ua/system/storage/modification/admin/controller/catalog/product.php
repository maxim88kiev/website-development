...
class ControllerCatalogProduct extends Controller {
    ...
    // OCFilter end
		...
		$data['layouts'] = $this->model_design_layout->getLayouts();
					...
					// XD Stickers start
						...
						$data['xd_stickers_product'] = array();
						$data['tab_xd_stickers'] = $this->language->get('tab_xd_stickers');
						$data['entry_xd_stickers'] = $this->language->get('entry_xd_stickers');
						$data['xd_stickers'] = $this->model_extension_module_xd_stickers->getCustomXDStickers();
						if (isset($this->request->post['product_layout'])) {
							$data['xd_stickers_product'] = $this->request->post['xd_stickers'];
						} elseif (isset($this->request->get['product_id'])) {
							$xd_stickers_products = array();
							$xd_stickers_products = $this->model_extension_module_xd_stickers->getCustomXDStickersProduct($this->request->get['product_id']);
							foreach ($xd_stickers_products as $xd_stickers_product) {
								$data['xd_stickers_product'][] = $xd_stickers_product['xd_sticker_id'];
							}
						} else {
							$data['xd_stickers_product'] = array();
						}
	...
	protected function validateForm() {
		...
		foreach ($this->request->post['product_description'] as $language_id => $value) {
			...
			if (false && (utf8_strlen($value['meta_title']) < 3) || (utf8_strlen($value['meta_title']) > 255)) {
