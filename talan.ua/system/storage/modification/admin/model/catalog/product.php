...
class ModelCatalogProduct extends Model {
    ...
    if (!empty($data['seo_canonical'])) {
      $this->db->query("UPDATE " . DB_PREFIX . "product SET seo_canonical = '" . $this->db->escape($data['seo_canonical']) . "' WHERE product_id = '" . (int)$product_id . "'");
        ...
        if ($this->config->get('mlseo_insertautoimgname') && !defined('GKD_UNIV_IMPORT')) {
      ...
      if ($this->config->get('mlseo_multistore')) {
        ...
        $this->model_catalog_seo_package->setSeoDescriptions('product', $data, $product_id);
      }
		...
		if (isset($data['product_recurring'])) {
			foreach ($data['product_recurring'] as $recurring) {
				$this->db->query("INSERT INTO `" . DB_PREFIX . "product_recurring` SET `product_id` = " . (int)$product_id . ", customer_group_id = " . (int)$recurring['customer_group_id'] . ", `recurring_id` = " . (int)$recurring['recurring_id']);
					...
					// XD Stickers start
						$this->db->query("DELETE FROM " . DB_PREFIX . "xd_stickers_product WHERE product_id = " . (int)$product_id);
						// var_dump($data['xd_stickers']);
						if (isset($data['xd_stickers'])) {
							foreach ($data['xd_stickers'] as $xd_sticker) {
								$this->db->query("INSERT INTO " . DB_PREFIX . "xd_stickers_product SET product_id = " . (int)$product_id . ", xd_sticker_id = ".(int)$xd_sticker['xd_sticker_id']);
							}
						}
					// XD Stickers end
    ...
    if (!empty($data['seo_canonical'])) {
      $this->db->query("UPDATE " . DB_PREFIX . "product SET seo_canonical = '" . $this->db->escape($data['seo_canonical']) . "' WHERE product_id = '" . (int)$product_id . "'");
        ...
        if ($this->config->get('mlseo_editautoimgname') && !defined('GKD_UNIV_IMPORT')) {
          $this->load->model('tool/seo_package');
          $seo_image_name = $this->model_tool_seo_package->transformProduct($this->config->get('mlseo_product_image_name_pattern'), $this->config->get('config_language_id'), $data);
