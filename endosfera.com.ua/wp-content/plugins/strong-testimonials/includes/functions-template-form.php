...
//            echo '</pre>';
				printf(
					'<textarea aria-label="like-4" id="wpmtst_%s" name="%s" class="%s" placeholder="%s" %s tabindex="0">%s</textarea>',
					esc_attr( $field['name'] ),
					esc_attr( $field['name'] ),
					esc_attr( wpmtst_field_classes( $field['input_type'], $field['name'] ) ),
                    esc_attr( $field['placeholder'] ),
                    esc_attr( wpmtst_field_required_tag( $field ) ),
					//esc_attr(  str_replace('"', "", wpmtst_field_placeholder( $field )) ),
					esc_textarea( $value )
				);
				break;
			...
			case 'file':
				echo '<div class="field-wrap">';
				echo '<input aria-label="like-3" id="wpmtst_' . esc_attr( $field['name'] ) . '" type="file" name="' . esc_attr( $field['name'] ) . '"' . esc_attr( wpmtst_field_required_tag( $field ) ) . ' tabindex="0">';
				echo '</div>';
				break;
			...
			case 'checkbox':
				...
				printf(
					'<span><input aria-label="like-2" id="wpmtst_%s" type="%s" class="%s" name="%s" %s %s tabindex="0"></span>',
					esc_attr( $field['name'] ),
		    ...
		    default: // text, email, url
			    printf( '<span></span><input aria-label="like" id="wpmtst_%s" type="%s" class="%s" name="%s" %s %s %s tabindex="0">',
			            $field['name'],
			            $field['input_type'],
...
function wpmtst_form_submit_button( $preview = false ) {
	...
	<div class="form-field wpmtst-submit">
		<label><input type="<?php echo $preview ? 'button' : 'submit'; ?>" id="wpmtst_submit_testimonial" name="wpmtst_submit_testimonial" value="<?php esc_attr_e( wpmtst_get_form_message( 'form-submit-button' ) ); ?>" class="<?php echo esc_attr( apply_filters( 'wpmtst_submit_button_class', 'button' ) ); ?> reviews__form-button" tabindex="0"></label>
	</div>
	<?php
}
