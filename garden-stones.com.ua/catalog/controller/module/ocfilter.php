...
class ControllerModuleOCFilter extends Controller {
  ...
  public function initialise() {
    ...
    if (version_compare(VERSION, '2.2', '<') == true) {
  		...
  		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/module/ocfilter/module.tpl')) {
  			$template = $this->config->get('config_template') . '/template/module/ocfilter/module.tpl';
  		} else {
  			$template = 'default/template/module/ocfilter/module.tpl';
  		}
    } else {
  		...
  		$template = 'module/ocfilter/module';
    }
    ...
    foreach ($params as $option_id => $values) {
      ...
      foreach ($values as $value_id) {
        ...
        if ($option_id == 'm') {
          ...
          $query = $this->db->query("SELECT keyword FROM " . DB_PREFIX . "ocfilter_option_value WHERE value_id = '" . $this->db->escape($value_id) . "'");
        }
