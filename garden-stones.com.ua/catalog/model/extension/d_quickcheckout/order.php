...
class ModelExtensionDQuickcheckoutOrder extends Model {
    ...
    public function updateOrder($order_id, $data) {
        ...
        foreach ($data['products'] as $product) {
            ...
            $option_quantity_keys = 0;
            $option_param_kg = 0;
            $option_param_quantity = array();
            $option_param = array();
            $option_size = array();
            $option_price_kg = array();


            foreach ($product['option'] as $option_pr) {

                if($option_pr['name'] == 'Цена за кг'){
                    array_push($option_price_kg, round($option_pr['value'],2));
                }


                if($option_pr['name'] == 'Размеры'){
                    $option_size[] = array(
                        'name'  => $option_pr['name'],
                        'value' => $option_pr['value']
                    );
                }
                if($option_pr['type'] == 'checkbox'){
                    array_push($option_param, round($option_pr['price'],2));
                    $option_param_quantity[$option_pr['quantity']] = $option_pr['price'];
                }
            }

            if(empty($option_size)){
                $option_size[] = array(
                    'name'  => NULL,
                    'value' => NULL
                );
            ...
            $option_name_param = 1;
            $option_name_weight = 'грн/шт';
            if(!empty($option_param)){
                $option_name_weight = 'грн/кг';
                $option_name_param = 25;
                if(!empty($option_price_kg)){
				
				//$option_name_param = $option_price_kg[0];
				
                    if($option_price_kg[0] == 0){
						$option_name_param = 1;
						$option_name_weight = 'грн/шт';
					}else{
						$option_name_param = $option_price_kg[0];
                ...
                $product_price = 0;
                $product_price_option = 0;
                $product_price_total = 0;
                $product_price_option = array_sum($option_param);
                $product_price = $product['price'] - $product_price_option;
                asort($option_param);
                ksort($option_param_quantity);

                $option_quantity_keys = array_keys($option_param_quantity);

                $option_param_kg = $product['quantity'] * $option_name_param;


                if ($option_param_kg >= $option_quantity_keys[0] && $option_param_kg < $option_quantity_keys[1]) {
                    $product_price = $option_param_quantity[$option_quantity_keys[0]];
                } elseif ($option_param_kg >= $option_quantity_keys[1] && $option_param_kg < $option_quantity_keys[2]) {
                    $product_price = $option_param_quantity[$option_quantity_keys[1]];
                } elseif ($option_param_kg >= $option_quantity_keys[2]) {
                    $product_price = $option_param_quantity[$option_quantity_keys[2]];
                }

//                if($product['quantity'] >= 5 && $product['quantity'] < 21 ){
//                    $product_price = $option_param[0];
//                }elseif($product['quantity'] >= 21 && $product['quantity'] < 41 ){
//                    $product_price = $option_param[1];
//                }elseif($product['quantity'] >= 41 ){
//                    $product_price = $option_param[2];
//                }



                $product_price_total = $product['quantity'] * $product_price * $option_name_param .'грн/шт';
                $product_price = $product_price.$option_name_weight;
            }else{
                $product_price = round($product['price'], 1).$option_name_weight;
                $product_price_total = round($product['total'], 1) * $option_name_param . 'грн/шт';
            }
...
//            $this->db->query("INSERT INTO " . DB_PREFIX . "order_product SET order_id = '" . (int) $order_id . "', product_id = '" . (int) $product['product_id'] . "', name = '" . $this->db->escape($product['name']) . "', model = '" . $this->db->escape($product['model']) . "', quantity = '" . (int) $product['quantity'] . "', price = '" . (float) $product['price'] . "', total = '" . (float) $product['total'] . "', tax = '" . (float) $product['tax'] . "', reward = '" . (int) $product['reward'] . "'");
            ...
            price = '" . (float) $product_price . "', total = '" . (float) $product_price_total . "', tax = '" . (float) $product_price_total . "', 
            reward = '" . (int) $product['reward'] . "', option_product = '" . (string) $option_size[0]['name'] . " : " . $option_size[0]['value'] . "'");
...
//            $this->db->query("INSERT INTO " . DB_PREFIX . "order_total SET order_id = '" . (int) $order_id . "', code = '" . $this->db->escape($total['code']) . "', title = '" . $this->db->escape($total['title']) . "', `value` = '" . (float) $total['value'] . "', sort_order = '" . (int) $total['sort_order'] . "'");
            $this->db->query("INSERT INTO " . DB_PREFIX . "order_total SET order_id = '" . (int) $order_id . "', code = '" . $this->db->escape($total['code']) . "', title = '" . $this->db->escape($total['title']) . "', `value` = '" . (float) $total_price . "', sort_order = '" . (int) $total['sort_order'] . "'");
        }
        if(VERSION < '2.3.0.0'){
