		...
		if ($order_info) {
			...
			if (!$order_info['order_status_id'] && $order_status_id) {
				...
				foreach ($order_product_query->rows as $product) {
                    ...
                    $order_option_query = $this->db->query("SELECT option_product FROM " . DB_PREFIX . "order_product WHERE order_id = '" . (int)$order_id . "' AND order_product_id = '" . $product['order_product_id'] . "'");
                    $options_size = $order_option_query->rows[0] ["option_product"];
...
//						$order_option_query = $this->db->query("SELECT option_product FROM " . DB_PREFIX . "order_product WHERE order_id = '" . (int)$order_id . "' AND order_product_id = '" . $product['order_product_id'] . "'");
//                        $value = $order_option_query->rows[0] ["option_product"];
//                        $text .= chr(9) . '-' . (utf8_strlen($value) > 20 ? utf8_substr($value, 0, 20) . '..' : $value) . "\n";
