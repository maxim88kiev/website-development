...
class ControllerExtensionModuleTmdfilterProduct extends Controller {
		...
		$popularposts= $this->model_extension_blog->getPopularBlog(5);
		...
		'href'        => $this->url->link('extension/blog', 'blog_id=' . $post['blog_id']),
		...
		$results = $this->model_extension_blog->getBlogs($filter_data);
			...
			foreach ($results as $result) {
				...
				$data['recentposts'][] = array(
					...
					'date_added'  => date($this->language->get('fulldate_format_added'), strtotime($result['date_modified'])),
					'description' => utf8_substr(strip_tags(html_entity_decode($result[
					'description'], ENT_QUOTES, 'UTF-8')), 0, 75).'&nbsp; ' .' <a href="'.$this->url->link('extension/blog', 'blog_id=' . $result['blog_id']).'">'.$this->language->get('text_readmore').'</a>',
		...
		$commentss= $this->model_extension_blog->getLatestcomments($data['limit']);
	//	print_r($commentss); 
		foreach($commentss as $posts){
			...
			'href'  => $this->url->link('extension/blog', 'blog_id=' . $posts['blog_id'])
		...
		$tagss= $this->model_extension_blog->getLatesttags();
		 
		foreach($tagss as $tag){
			$data['tags'][] = array(
			'blog_id'  => $tag['blog_id'],			
			'tag'  => $tag['tag'],			
			'href'  => $this->url->link('extension/blog', 'blog_id=' . $tag['blog_id'])
			);
		} 
