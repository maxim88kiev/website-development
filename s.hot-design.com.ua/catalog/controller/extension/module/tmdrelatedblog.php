		...
		if (!empty($setting['blogs'])) {
			...
			foreach ($blogss as $blog_id) {
				...
				if ($blog_info) {
					...
					$comments = $this->model_extension_blog->Updatecomment($blog_info['blog_id']);
					...
					$data['blogss'][] = array(
						...
						'description' => utf8_substr(strip_tags(html_entity_decode($blog_info['description'], ENT_QUOTES, 'UTF-8')), 0, 150),
						'href'        => $this->url->link('extension/blog', 'blog_id=' . $blog_info['blog_id'])
					);
				}
			}
		}
