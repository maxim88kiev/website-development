...
class ControllerExtensionBlog extends Controller {
	...
	public function index() {
		...
		$data['breadcrumbs'][] = array(
			...
			$data['uploadimges'] = $this->url->link('extension/blog/uploadimage') ;
			/*setting*/
			...
			$blog_info = $this->model_extension_blog->getBlog($blog_id);
			...
			if ($blog_info) {
				...
				$data['breadcrumbs'][] = array(
				'text' => $blog_info['name'],
				'href' => $this->url->link('extension/blog', $url . '&blog_id=' . $this->request->get['blog_id'])
				);
				...
				$this->document->addLink($this->url->link('extension/blog', 'blog_id=' . $this->request->get['blog_id']), 'canonical');
				...
				$this->load->model('tool/image');
				if ($blog_info['image']) {
					$data['thumb'] = $this->model_tool_image->resize($blog_info['image'], 1170, 600);
				} else {
					$data['thumb'] = '';
				}
				...
				$tagss = explode(',',$blog_info['tag']);
					if(!empty($tagss))
					{
						foreach($tagss as $tag){
							$data['tags'][] = array(
							'tag'  => $tag,			
							'href'  => $this->url->link('extension/allblogcategory', 'tag=' . $tag)
							);
						} 
					}
				// New code 
				...
				$data['href'] = $this->url->link('extension/blog', '&blog_id=' . $this->request->get['blog_id']);
				...
				$data['comments'] = $this->model_extension_blog->Updatecomment($blog_id);
				$data['comment_info'] =array();
				$comment_infos = $this->model_extension_blog->getcomments($blog_id);
				...
				'comment' => strip_tags(html_entity_decode($info['comment'], ENT_QUOTES, 'UTF-8'))
				 ...
				 $this->model_extension_blog->updateViewed($blog_id);
				...
				if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/extension/blog')) {
					$this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/extension/blog', $data));
				} else {
				...
				$data['breadcrumbs'][] = array(
					...
					'href' => $this->url->link('extension/blog', $url . '&blog_id=' . $blog_id)
				);
		...
		public function addblogcomments() {	
			...
			{
			$this->model_extension_blog->Addblogcomment($this->request->post);
			...
			$json['comments'] = $this->model_extension_blog->Updatecomment($blog_id);
			}
			}
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/extension/comments/loadblogcomment')) {
			$this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/extension/comments/loadblogcomment', $data));
		} else {
			$this->response->setOutput($this->load->view('extension/comments/loadblogcomment', $data));
		}
		...
		if (!$json) {
			...
			move_uploaded_file($this->request->files['file']['tmp_name'], DIR_IMAGE.'catalog/commentmage/' . $file);

			// Hide the uploaded file name so people can not link to it directly.
			$this->load->model('tool/upload');

			$json['success'] = $this->language->get('text_upload');
			$json['file'] ='catalog/commentmage/'.$file;
			$file1=$this->model_tool_image->resize('catalog/commentmage/'.$file, $this->config->get('tmdblogsetting_image_comntthumb_width'), $this->config->get('tmdblogsetting_image_comntthumb_height'));
			$json['file1'] = $file1;
		}
