(function($) {
    ...
    let $deactivateForm  = jQuery('#slider-deactivateForm');
    ...
    let deactivateFormSubmitted = localStorage.getItem('slider-deactivateFormSubmitted');
    ...
    function submitForm(deactivate_options, id) {
        ...
        data.push({name: 'action', value: 'slider-deactivate'});
        ...
        if (!deactivateFormSubmitted) {
            localStorage.setItem('slider-deactivateFormSubmitted', true);
            deactivateFormSubmitted = true;

            jQuery.post('./controller/extension/module/elfsight_slider.php', jQuery.param(data)).then(function(result) {
