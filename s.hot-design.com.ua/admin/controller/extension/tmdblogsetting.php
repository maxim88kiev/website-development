<?php
class ControllerExtensionTmdblogsetting extends Controller {
	private $error = array();

	public function index() {
		$this->load->language('extension/tmdblogsetting');
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->response->redirect($this->url->link('extension/tmdblogsetting', 'user_token=' . $this->session->data['user_token'], true));
		}
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('extension/tmdblogsetting', 'user_token=' . $this->session->data['user_token'] . $url, true)
		);
		
		$data['action'] = $this->url->link('extension/tmdblogsetting', 'user_token=' . $this->session->data['user_token'], true);

		$data['cancel'] = $this->url->link('extension/tmdblogsetting', 'user_token=' . $this->session->data['user_token'] . $url, true);
		...
		$this->response->setOutput($this->load->view('extension/tmdblogsetting_form', $data));
	}

	protected function validateForm() {
		if (!$this->user->hasPermission('modify', 'extension/tmdblogsetting')) {
